...
d.text((40, 68), "跨凹槽插入：左边两脚是一对、右边两脚是一对；按下时左右导通", font=f_sub, fill="#333333")
...
d.line((pin["tl"][0], pin["tl"][1] + 10, pin["bl"][0], pin["bl"][1] - 10), fill="#224466", width=2)
d.line((pin["tr"][0], pin["tr"][1] + 10, pin["br"][0], pin["br"][1] - 10), fill="#224466", width=2)
d.text((pin["tl"][0] + 12, (pin["tl"][1] + pin["bl"][1]) // 2 - 8), "同一对脚", font=f_small, fill="#224466")
d.text((pin["tr"][0] + 12, (pin["tr"][1] + pin["br"][1]) // 2 - 8), "同一对脚", font=f_small, fill="#224466")
...
# 黑：GND 到 行20 列g（与行20 列f 同节点），再到 - 轨
gnd_x = col_x["g"] + COL_W // 2
d.line((gnd_x, ROWS[20] + ROW_H // 2, gnd_x, 150), fill="#111111", width=4)
d.line((gnd_x, 150, 152, 150), fill="#111111", width=4)
d.line((152, 150, 152, ROWS[20] + ROW_H // 2), fill="#111111", width=4)
d.text((gnd_x + 10, 124), "黑线 到 - 轨(GND)", font=f_label, fill="#111111")
d.text((gnd_x + 10, 154), "接行20列g", font=f_small, fill="#111111")
