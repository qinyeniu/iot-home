...
## 真实断电复测（2026-09-10 23:51）

- 用户拔掉 COM6 USB 供电 10 秒后重新插入。
- 随后诊断固件 8 轮主动扫描均能看到 5–6 个周围 2.4GHz AP（如 `TP-LINK_2558HGM`、`ZYZ`、`hayate-wifi`、`1722`）。
- 同时间家庭 2.4GHz `CU_0D84` 对电脑和板子均不可见，只有 `CU_0D84_5G` 可见。
- 结论：板端 RF 并非永久硬件损坏；此前所有 AP 扫描为 0 属于只有真实断电才能恢复的 RF/WiFi 卡死状态。
- `IOT_RF_TEST` 未被电脑看到，但当时电脑扫描列表不完整，该发射测试结论暂不定性。
